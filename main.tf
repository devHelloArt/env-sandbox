...
locals {
  ...
  k8s_cluster_name = "htt-dev-cluster"
}
...
module "aws-network" {
  source = "github.com/devHelloArt/module-aws-network"

  env_name              = local.env_name
  vpc_name              = "htt-dev-VPC"
...
module "aws-kubernetes-cluster" {
  source = "github.com/devHelloArt/module-aws-kubernetes"

  ms_namespace       = "httClientService"
  env_name           = local.env_name
  aws_region         = local.aws_region
  cluster_name       = local.k8s_cluster_name
...
module "argo-cd-server" {
  source = "github.com/devHelloArt/module-argo-cd"
...
module "aws-databases" {
  source = "github.com/devHelloArt/ms-aws-db"

  mysql_identifier = "mysql-sandbox"
...
# Create Traefik
module "traefik" {
  source = "github.com/devHelloArt/ms-traefik"
